...
Like with the *required* elements of `ggplot`, styling is added layer by layer. You can scale the x-axis by specifying the intervals you want to show and how you want the labels to look. You can add a title and caption to the graph and lable the axes. And you can modify the theme by adding or removing individual elements.
...
It's not necessary to know the ins and outs of each of these style modifications – there are way to many to commit to memory at this point, and you can always look them up in[ `ggplot`'s documentation](https://ggplot2.tidyverse.org/).
...
Note that the following code looks pretty similar to the what we've seen so far, with the exception of our `aes` tweak and the removal of the color definition from our `geom_line()`.
